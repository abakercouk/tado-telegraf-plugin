...
func (plugin *Tado) dumpZone(ctx context.Context, u *gotado.User, h *gotado.Home, z *gotado.Zone, a telegraf.Accumulator) {
	...
	tags["home"] = h.Name
	tags["zone"] = z.Name
	tags["tadoModeState"] = state.TadoMode
	tags["powerState"] = string(state.Setting.Power)
	...
	if string(state.Setting.Power) == "ON" {
		a.AddError("Setting power field = 1")
		fields["power"] = 1
	} else {
		a.AddError("Setting power field = 0")
		fields["power"] = 0
	}

	if state.TadoMode == "HOME" {
		a.AddError("Setting tadoMode field = 1")
		fields["tadoMode"] = 1
	} else {
		a.AddError("Setting tadoMode field = 0")
		fields["tadoMode"] = 0
	}
